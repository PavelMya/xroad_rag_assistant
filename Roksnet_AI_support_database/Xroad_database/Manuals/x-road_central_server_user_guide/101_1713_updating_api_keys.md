...
```bash
curl -X PUT -u : https://localhost:4000/api/v1/api-keys/5 --data '["XROAD_SECURITY_OFFICER”,”XROAD_REGISTRATION_OFFICER"]' --header "Content-Type: application/json" -k
{
  "id": 5,
  "roles": [
...
```bash
curl -X DELETE -u : https://localhost:4000/api/v1/api-keys/5  -k

...
1.  SS administrator sends a REST request to perform some kind of configuration action. REST client should
    ...
    - 2.2 Send request to URL corresponding to the desired action,
     for example `https://:4000/api/v1/clients` to list clients.
    ...
    `Authorization: X-Road-ApiKey token=`
    ...
    - Example using "curl" command: `curl --header "Authorization: X-Road-apikey token=" "https://:4000/api/v1/clients" -k`
